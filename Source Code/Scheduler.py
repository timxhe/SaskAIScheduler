...
def scheduler(currentSettings, res_List, opSettings, test_time, path, diagnostics):
   ...
   # Total number of residents
    ...
    # Current logic is to minimize differences in cross covering in IM residents and off-service residents
    ...
    model.Add(scheduleMetric == 2*(averageLoad + comboScore) + avgDensity + scoreAdjust)
    ...
    ws.write(9 + opSettings.num_days, 0, "Average density: " + str(solver.Value(avgDensity)))
    ws.write(10 + opSettings.num_days, 0, "Adjusted CC score: " + str(solver.Value(scoreAdjust)))

    # Each resident stats
    ws.write(12 + opSettings.num_days, 0, "Resident Stats", bold)
    x = 13 + opSettings.num_days
