...
def calcSettings(currentSettings, res_List):
    ...
    all_weekdays = [d for d in all_days if (d % 7 >= 0) and (d % 7 <= 4)]
    all_weekends = [d for d in all_days if (d % 7 >= 5) and (d % 7 <= 6)]
    ...
    return op.opSettings(num_res, num_teams, num_days, num_weeks, all_res, all_teams, all_acteams, all_days, all_weeks, max_freq, max_shifts, stat_days, all_weekends, all_weekdays)