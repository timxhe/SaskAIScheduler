...
def infeasibleScheduler(currentSettings, res_List, opSettings, test_time, path):
    ...
    for n in opSettings.all_res:
        if res_List[n].is_IM == False:
            for d in opSettings.all_days:
                model.Add((sum(shifts[(n, d, t)] for t in opSettings.all_teams) - shifts[n, d, res_List[n].res_team] -
                           shifts[n, d, 3]) == 0)
    ...
    model.Add(scheduleMetric == (flyins + averageLoad + comboScore))
    ...
    # Each resident stats
    ws.write(12 + opSettings.num_days, 0, "Resident Stats", bold)
    x = 13
    ws.write(x + opSettings.num_days, 0, "Resident Name", bold)
    ws.write(x + opSettings.num_days, 1, "Resident Team", bold)
    ws.write(x + opSettings.num_days, 2, "Resident Days Worked", bold)
    ws.write(x + opSettings.num_days, 3, "Resident Weekend Shifts", bold)
    ws.write(x + opSettings.num_days, 4, "Resident Raw Load", bold)
    ws.write(x + opSettings.num_days, 5, "Resident Corrected Load", bold)
    ws.write(x + opSettings.num_days, 6, "Resident Load", bold)
    ws.write(x + opSettings.num_days, 7, "Combination Days", bold)

    for n in opSettings.all_res:
        r = n + opSettings.num_days + x + 1
